...
## E-121 — a green public sync deleted public-owned assets, and its receipt was not a receipt

- **Symptom:** public PR #34 passed all four CI jobs while its 787-file output deleted current brand
  assets and public documentation inherited from the 0.71.5 public tree. Private PR #66 was then
  reported as deterministic and transactional, but the same reviewed head still wrote
  `new Date().toISOString()`, swallowed an invalid prior manifest, accepted unreadable configuration,
  and performed in-place delete/copy/write operations without rollback.
- **Root cause:** the synchronizer treated “not present in this HQ staging output” as permission to
  delete a public path. Tests drove lower-level helpers with injected safe values instead of the real
  `executeSync` entry point, so they proved a path the operator did not run. Temporary staging
  protected source assembly but not the destination once mutation began.
- **Second defect found by the real public dry-run:** public `main` predates the ownership ledger
  and still contains historical HQ-only files. Pure preservation correctly blocks them at the final
  secret gate, but leaves no usable migration path. The repair adds a one-use bootstrap tied to the
  exact reviewed public HEAD; it prints all planned deletions and is refused after a ledger exists.
- **Why green CI did not refute it:** runtime, lint and coverage jobs do not judge the ownership of
  a public-only asset. The helper tests also replaced the real wall clock and never injected a fault
  between destination operations. Every assertion passed against a narrower system than the one
  named in the report.
- **Fix:** use a strict deterministic `.sparda-managed.json` ownership ledger. Delete only paths
  named by the prior ledger and absent from new staging; preserve all public-owned paths. Validate
  exact remotes, clean roots, configuration, ledger schema, paths, links, secrets, imports and
  identity before mutation. Snapshot destination bytes and modes, then rollback on any synchronous
  application failure. Drive all adversarial cases through the real entry point and mutation-test
  the guards.
- **Scope correction:** “transactional” means rollback on caught synchronous errors. Sudden power
  loss or process termination is not proven atomic and is not claimed.
- **Rule:** **a green test of a helper is not evidence about an entry point it bypasses.** For a
  destructive synchronizer, test the invoked boundary, inject failure after every mutation phase,
  and require an explicit prior ownership receipt before deletion.


## E-122 — the final-tree gate caught private verification metadata selected by `tests/**`

- **Symptom:** an exact-head bootstrap dry-run against public `main` stopped before mutation with
  review hits in `tests/mutation/run.mjs`, two internal documentation indexes and the old French
  explanation. The source and destination repositories remained clean.
- **Root cause:** the allowlist classified files only by broad path families. The mutation runner
  is legitimately public, but private records inside it named valve files and private tests. The
  three documents had been made public by path even though their content still described internal
  process or unreleased strategy.
- **Why tests were green:** fixture trees used clean synthetic documents and did not assemble the
  current repository against the current public tree. Unit coverage proved the scanner; only the
  full real dry-run supplied the content that needed scanning.
- **Fix:** move private mutant records to a schema-checked extension under default-denied
  `internal/**`; keep the public runner self-sufficient. Remove internal indexes from the public
  allowlist and replace the linked French explanation with a curated public overlay. Add a test that
  scans the exact public runner and overlay. No exception was added.
- **Rule:** **an allowlisted path is not automatically public-safe content.** Scan the assembled
  bytes, including test metadata and overlays, against the real destination before granting any
  write authority.


## E-123 — a managed-only deletion plan classified the Claude plugin contract as disposable

- **Symptom:** the real exact-head public dry-run planned to delete
  `.claude-plugin/marketplace.json`. Inspection then found the surviving source manifest and the
  Claude Code installation README still named the legacy GitHub owner, even though the public
  repository is `zakariagharzouli/sparda`.
- **Root cause:** the product identity gate scanned README, action, source, extensions,
  integrations and selected docs, but omitted `.claude-plugin/**`. Its legacy-owner expression
  also lacked JSON punctuation as a valid URL terminator. Synthetic fixtures therefore never
  asserted that the plugin marketplace manifest was a managed public product surface.
- **Fix:** explicitly allowlist `.claude-plugin/marketplace.json`, migrate its owner URL and the
  documented marketplace command, scan `.claude-plugin/**` in the assembled final tree, and add
  two independent mutation guardians: one removes that surface, the other removes the JSON
  punctuation boundary.
- **Rule:** **a deletion list is an identity audit, not cleanup noise.** Every public product
  manifest must be named, staged and checked through the exact final-tree entry point.


## E-124 — a README oracle walked the installed workspace instead of the public file set

- **Symptom:** the PR CI failed on Linux while local Windows tests passed. The README-link test
  called the validator with the repository root, so its recursive safety walk reached the ordinary
  npm symlinks under `node_modules/.bin` and rejected them. Coverage failed on the same test.
- **Root cause:** the assertion claimed to validate the assembled public README, but its oracle was
  the developer workspace. Windows npm shims are regular files while Linux npm shims are symlinks,
  making the bypass both semantically wrong and OS-dependent.
- **Fix:** derive the exact tracked allowlisted set with the production partitioner, add only the
  four curated overlay destinations, and pass that closed `Set` to the link validator. The test now

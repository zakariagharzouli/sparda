...
const BASE_MUTANTS = [
  {
    desc: 'identity risk: synchronous admission',
    file: 'src/ubg/session-admission.js',
    find: 'fn.async ||',
    repl: 'false ||',
    test: 'tests/session-admission.test.js',
  },
  {
    desc: 'identity risk: request identity',
    file: 'src/ubg/session-admission.js',
    find: '!field?.startsWith(prefix)',
    repl: 'false',
    test: 'tests/session-admission.test.js',
  },
  {
    desc: 'identity risk: continuation identity',
    file: 'src/ubg/session-admission.js',
    find: 'member(s.argument.callee) === next',
    repl: 'true',
    test: 'tests/session-admission.test.js',
  },
  {
    desc: 'identity risk: continuation arguments',
    file: 'src/ubg/session-admission.js',
    find: 's.argument.arguments.length === 0',
    repl: 'true',
    test: 'tests/session-admission.test.js',
  },
  {
    desc: 'identity risk: response identity',
    file: 'src/ubg/session-admission.js',
    find: 'name === `${res}.redirect`',
    repl: 'true',
    test: 'tests/session-admission.test.js',
  },
  {
    desc: 'identity risk: denial status',
    file: 'src/ubg/session-admission.js',
    find: '[401, 403].includes(call.arguments[0].value)',
    repl: 'true',
    test: 'tests/session-admission.test.js',
  },
  {
    desc: 'identity risk: literal redirect',
    file: 'src/ubg/session-admission.js',
    find: "call.arguments[0].type === 'StringLiteral'",
    repl: 'true',
    test: 'tests/session-admission.test.js',
  },
  {
    desc: 'identity risk: bounded admission',
    file: 'src/ubg/session-admission.js',
    find: 'body.length > 4',
    repl: 'false',
    test: 'tests/session-admission.test.js',
  ...
  {
    desc: 'identity risk: side effects in logs',
    file: 'src/ubg/session-admission.js',
    find: "s.expression.arguments.some((a) => a.type !== 'StringLiteral')",
    repl: 'false',
    test: 'tests/session-admission.test.js',
  },
  {
    desc: 'identity risk: authorization admission',
    file: 'src/ubg/session-admission.js',
    find: 'authorizationProven: null,',
    repl: 'authorizationProven: true,',
    test: 'tests/session-admission.test.js',
  },
  {
    desc: 'identity risk: registered route chain',
    file: 'src/ubg/identity-risk.js',
    find: 'if (registered.length !== 2) return null;',
    repl: 'if (false) return null;',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: inline denial',
    file: 'src/ubg/identity-risk.js',
    find: '(n.meta?.bodyDenies || n.meta?.bodyVerifies || n.meta?.bodyRedirects)',
    repl: 'false',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: framework boundary',
    file: 'src/ubg/identity-risk.js',
    find: "graph.meta?.framework !== 'express'",
    repl: 'false',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: facts budget',
    file: 'src/ubg/identity-risk.js',
    find: 'facts.length > 200000',
    repl: 'false',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: filter role',
    file: 'src/ubg/identity-risk.js',
    find: "f.role !== 'filter' ||",
    repl: 'false ||',
    test: 'tests/identity-risk.test.js',
  ...
  {
    desc: 'identity risk: resolved path',
    file: 'src/ubg/identity-risk.js',
    find: "f.state !== 'resolved' ||",
    repl: 'false ||',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: normalizer uncertainty',
    file: 'src/ubg/identity-risk.js',
    find: "f.path.some((step) => /\\(\\)$/.test(step) && step !== 'parseInt()')",
    repl: 'false',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: source identity',
    file: 'src/ubg/identity-risk.js',
    find: 'f.source.name !== guard.meta.sessionAdmission.name',
    repl: 'false',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: selector destination',
    file: 'src/ubg/identity-risk.js',
    find: '!/^filter\\.[A-Za-z_][\\w$]*$/.test(f.destination)',
    repl: 'false',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: symbolic target',
    file: 'src/ubg/identity-risk.js',
    find: 'occurrence.symbolicTarget ||',
    repl: 'false ||',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: known target',
    file: 'src/ubg/identity-risk.js',
  ...
  {
    desc: 'identity risk: occurrence owner scope',
    file: 'src/ubg/identity-risk.js',
    find: 'occurrence.ownerScoped ||',
    repl: 'false ||',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: effect reachability',
    file: 'src/ubg/identity-risk.js',
    find: '!reached.includes(effect)',
    repl: 'false',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: stale guard',
    file: 'src/ubg/identity-risk.js',
    find: 'e.guard === guard.id &&',
    repl: 'true &&',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: stale session',
    file: 'src/ubg/identity-risk.js',
    find: 'e.session === guard.meta.sessionAdmission.name &&',
    repl: 'true &&',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: stale source',
    file: 'src/ubg/identity-risk.js',
    find: "['body', 'params', 'query'].includes(e.source.origin)",
    repl: 'true',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: unconfirmed violation',
    file: 'src/ubg/identity-risk.js',
    find: 'authorizationViolation: null,',
    repl: 'authorizationViolation: true,',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: unconfirmed exploit',
    file: 'src/ubg/identity-risk.js',
    find: 'exploitability: null,',
    repl: 'exploitability: true,',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: compiler wiring',
    file: 'src/ubg/compile.js',
    find: 'attachIdentityRiskEvidence(graph, lifted.facts);',
    repl: '/* detached */',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: public grader wiring',
    file: 'src/ubg/apocalypse.js',
    find: 'findings.push(...identityRisks);',
    repl: '/* detached */',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: admission extraction',
    file: 'src/ubg/extract.js',
    find: 'const admission = sessionAdmission(fnNode);',
    repl: 'const admission = null;',
    test: 'tests/identity-risk.test.js',
  },
  {
    desc: 'identity risk: helper admission metadata',
  ...
  {
    desc: 'identity risk: registered admission metadata',
    file: 'src/ubg/translate.js',
    find: 'role: step.role,\n        async: scan.async,\n        ...(scan.sessionAdmission ? { sessionAdmission: scan.sessionAdmission } : {}),',
    repl: 'role: step.role,\n        async: scan.async,',
    test: 'tests/identity-risk.test.js',
  ...
  {
    desc: 'local return paths: immutable-producer',
    file: 'src/ubg/local-return-paths.js',
    find: "statement.kind !== 'const'",
    repl: 'false',
    test: 'tests/local-return-paths.test.js',
  },
  {
    desc: 'local return paths: async-producer',
    file: 'src/ubg/local-return-paths.js',
    find: 'fn.async ||',
    repl: 'false ||',
    test: 'tests/local-return-paths.test.js',
  ...
  {
    desc: 'local return paths: parameterless-producer',
    file: 'src/ubg/local-return-paths.js',
    find: 'fn.params.length ||',
    repl: 'false ||',
    test: 'tests/local-return-paths.test.js',
  },
  {
    desc: 'local return paths: unambiguous-name',
    file: 'src/ubg/local-return-paths.js',
    find: 'if (counts.get(name) !== 2) continue;',
    repl: 'if (false) continue;',
    test: 'tests/local-return-paths.test.js',
  },
  {
    desc: 'local return paths: call-order',
    file: 'src/ubg/local-return-paths.js',
    find: 'call.start <= fn.end',
    repl: 'false',
    test: 'tests/local-return-paths.test.js',
  },
  {
    desc: 'local return paths: argument-positions',
    file: 'src/ubg/local-return-paths.js',
    find: 'call.arguments.length ||',
    repl: 'false ||',
    test: 'tests/local-return-paths.test.js',
  },
  {
    desc: 'local return paths: bounded-tree',
    file: 'src/ubg/local-return-paths.js',
    find: "if (--fuel < 0) throw new Error('budget');",
    repl: '/* bound removed */',
    test: 'tests/local-return-paths.test.js',
  },
  {
    desc: 'local return paths: bounded-alternatives',
    file: 'src/ubg/local-return-paths.js',
    find: 'alternatives.length <= 8 &&',
    repl: 'true &&',
    test: 'tests/local-return-paths.test.js',
  },
  {
    desc: 'local return paths: pure-return-values',
    file: 'src/ubg/local-return-paths.js',
    find: 'alternatives.every((n) => value(n))',
    repl: 'true',
    test: 'tests/local-return-paths.test.js',
  },

  {
    desc: 'authorization policy: stale-source',
    file: 'src/ubg/authorization-policy.js',
    find: 'if (graph?.meta?.sourceHash !== policy.sourceHash)',
    repl: 'if (false)',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: route-identity',
    file: 'src/ubg/authorization-policy.js',
    find: 'f.entrypoint === rule.entrypoint &&',
    repl: 'true &&',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: table-identity',
    file: 'src/ubg/authorization-policy.js',
    find: 'f.table === rule.table &&',
    repl: 'true &&',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: access-identity',
    file: 'src/ubg/authorization-policy.js',
    find: 'f.access === rule.access &&',
    repl: 'true &&',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: symbolic-target',
    file: 'src/ubg/authorization-policy.js',
    find: '!f.symbolicTarget &&',
    repl: 'true &&',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: effect-identity',
    file: 'src/ubg/authorization-policy.js',
    find: "nodes.get(f.effect)?.kind === 'effect'",
    repl: 'true',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: origin-identity',
    file: 'src/ubg/authorization-policy.js',
    find: 'o.origin === rule.source.origin &&',
    repl: 'true &&',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: source-field',
    file: 'src/ubg/authorization-policy.js',
    find: 'o.name === rule.source.name',
    repl: 'true',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: middleware-file',
    file: 'src/ubg/authorization-policy.js',
    find: 'n.loc?.file === rule.middleware.file &&',
    repl: 'true &&',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: middleware-line',
    file: 'src/ubg/authorization-policy.js',
    find: 'n.loc?.line === rule.middleware.line &&',
    repl: 'true &&',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: middleware-symbol',
    file: 'src/ubg/authorization-policy.js',
    find: 'n.label === rule.middleware.symbol',
    repl: 'true',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: opaque-chain',
    file: 'src/ubg/authorization-policy.js',
    find: 'nodes.get(e.to).meta?.opaque',
    repl: 'false',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: unknown-headline',
    file: 'src/ubg/authorization-policy.js',
    find: 'violations: null,',
    repl: 'violations: 0,',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: runtime-admission',
    file: 'src/ubg/authorization-policy.js',
    find: 'satisfied: null,\n    runtimeViolation: null,',
    repl: 'satisfied: null,\n    runtimeViolation: true,',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: automatic-admission',
    file: 'src/ubg/authorization-policy.js',
    find: '    automaticDetection: false,\n  });',
    repl: '    automaticDetection: true,\n  });',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: byte-budget',
    file: 'src/ubg/authorization-policy.js',
    find: 'if (policyBytes > 65536)',
    repl: 'if (false)',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: partial-unknown-violations',
    file: 'src/ubg/authorization-policy.js',
    find: "violations: checks.some((c) => c.state === 'unknown')",
    repl: 'violations: false',
    test: 'tests/authorization-policy.test.js',
  },
  {
    desc: 'authorization policy: partial-unknown-satisfied',
    file: 'src/ubg/authorization-policy.js',
    find: "satisfied: checks.some((c) => c.state === 'unknown')",
    repl: 'satisfied: false',
    test: 'tests/authorization-policy.test.js',
  },

  {
    desc: 'config closure: stop inheriting local configuration',
    file: 'src/ubg/extract.js',
    find: 'inherited = read(fs.existsSync(base) ? base : `${base}.json`);',
    repl: 'inherited = { baseUrl: null, paths: {}, pathsOrigin: null };',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: use child origin for parent options',
    file: 'src/ubg/extract.js',
    find: 'const origin = path.dirname(filename);',
    repl: 'const origin = dir;',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: merge paths instead of replacing',
    file: 'src/ubg/extract.js',
    find: 'paths: options.paths, pathsOrigin: origin',
    repl: 'paths: { ...inherited.paths, ...options.paths }, pathsOrigin: origin',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: admit a cycle',
    file: 'src/ubg/extract.js',
    find: "if (active.has(physical)) fail('config-cycle');",
    repl: '/* removed by config mutant */',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: remove file bound',
    file: 'src/ubg/extract.js',
    find: "if (configurationFiles.length >= 16) fail('config-file-budget');",
    repl: '/* removed by config mutant */',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: disable aggregate byte bound',
    file: 'src/ubg/extract.js',
    find: '4 * 1024 * 1024',
    repl: 'Number.MAX_SAFE_INTEGER',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: admit baseUrl reanchoring',
    file: 'src/ubg/extract.js',
    find: "fail('config-inherited-paths-base-override');",
    repl: 'void 0;',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: hide uncertainty',
    file: 'src/ubg/extract.js',
    find: 'unknown: error.message,',
    repl: 'unknown: null,',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: claim the whole configuration certified',
    file: 'src/ubg/extract.js',
    find: 'fullConfigurationValidated: false,',
    repl: 'fullConfigurationValidated: true,',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: admit unterminated comments',
    file: 'src/ubg/extract.js',
    find: "if (i >= src.length) throw new Error('unterminated-comment');",
    repl: '/* removed by config mutant */',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: concatenate tokens around comments',
    file: 'src/ubg/extract.js',
    find: "out += ' '; // comments separate tokens; they must never concatenate them",
    repl: '/* removed by config mutant */',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: ignore exact alias priority',
    file: 'src/ubg/extract.js',
    find: 'selected = { targets, mid: null };',
    repl: 'selected = selected;',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: reverse wildcard precedence',
    file: 'src/ubg/extract.js',
    find: 'if (pre.length > selectedPrefix)',
    repl: 'if (pre.length < selectedPrefix)',
    test: 'tests/config-closure.test.js',
  },
  {
    desc: 'config closure: omit inherited config bytes from graph identity',
    file: 'src/ubg/compile.js',
    find: '...configurationResolution(cwd).flatMap((configuration) => configuration.files),',
    repl: '...[],',
    test: 'tests/config-closure.test.js',
  },
...
function loadMutationExtensions() {
  const extensionPath = f('internal/mutation-extensions.json');
  if (!fs.existsSync(extensionPath)) return [];
  const extensions = JSON.parse(fs.readFileSync(extensionPath, 'utf8'));
  if (!Array.isArray(extensions)) {
    throw new Error('internal mutation extensions must be a JSON array');
  }
  for (const [index, mutant] of extensions.entries()) {
    for (const field of ['desc', 'file', 'find', 'repl', 'test']) {
      if (typeof mutant?.[field] !== 'string' || mutant[field].length === 0) {
        throw new Error(`internal mutation extension ${index} has invalid ${field}`);
      }
    }
  }
  return extensions;
...
// The public runner stays autonomous. HQ may add default-denied guardians through this
// data-only extension without publishing private file paths or private test architecture.
export const MUTANTS = [...BASE_MUTANTS, ...loadMutationExtensions()];
